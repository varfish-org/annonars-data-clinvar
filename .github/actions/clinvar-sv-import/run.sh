...
for x in \
  $CLINVAR_JSONL_DIR/clinvar-data-extract-vars-*/clinvar-variants-$genome_release-seqvars.jsonl.gz \
  $CLINVAR_JSONL_DIR/clinvar-data-extract-vars-*/clinvar-variants-$genome_release-strucvars.jsonl.gz; do \
    zcat $x \
    | tr "'" '"' \
    | sed -e 's/None/null/g' \
